...
ARGS, CONFIG = prepare_figure('figED8', 'Reproduce Extended Data Figure 8 using the linear budget.')
...
os.environ["DA_LAYOUT_MODES"] = "beeswarm"
os.environ["DA_LINEAR_PLOT_GROUPS"] = "three"
os.environ["DA_BASE_DIR"] = str(CACHE_ROOT / "dynamic_adjustment" / "linear_budget")
os.environ["DA_VARIANT_TAGS"] = "linearbudget_chwcalendar31d"
# 若需要继续调整 Fig.5–Fig.7 与 linear budget 图的文字大小，请修改 cell 6 中的 FIG567_FONT_SCALE。
LINEAR_PLOT_STYLE_CONFIG = {
    "title_template": "{var} | {component}",
    "var_labels": {"tmax": "TMAX_CH", "tmin": "TMIN_CH"},
    "component_labels": {
        "dyn": "Dynamical",
        "thermo": "Thermodynamic",
        "dyn_linear": "Dynamical",
        "thermo_linear": "Thermodynamic",
        "deficit_linear": "Deficit",
    },
    "ylabel": "Trend (°C yr$^{-1}$)",
    "band_ylabel": "Models >\nERA5 (%)",
    ...
    "mme_label": "MME",
    "title_pad": 10,
    "title_fontsize": FIG567_PANEL_TITLE_PT-4,
    "title_fontweight": "normal",
    "panel_label_x": PANEL_LABEL_X_WITH_TITLE,
    "panel_label_y": PANEL_LABEL_Y_WITH_TITLE,
    "panel_label_fontsize": FIG567_PANEL_LABEL_PT-4,
    "panel_label_fontweight": "bold",
    "annotation_fontsize": FIG567_ANNOTATION_PT,
    "main_tick_fontsize": FIG567_AXIS_TICK_PT,
    "main_xtick_fontsize": FIG567_AXIS_TICK_PT,
    "ylabel_fontsize": FIG567_AXIS_LABEL_PT,
    "empty_member_label_fontsize": FIG567_LEGEND_PT,
    "band_xtick_fontsize": FIG567_AXIS_TICK_PT-2,
    "band_value_fontsize": FIG567_AXIS_TICK_PT-5,
    "band_ylabel_fontsize": FIG567_CBAR_LABEL_PT-5,
    "legend_label_fontsize": FIG567_LEGEND_PT+2,
    "member_key_title_fontsize": FIG567_LEGEND_PT,
    "member_label_fontsize": FIG567_LEGEND_PT+2,
    "output_dpi": 320,
    "rcParams": FINAL_DA_RCPARAMS,
    "figure_size": [23.8, 18.2],
    "subplots_left": 0.055,
    "subplots_right": 0.965,
    "subplots_top": 0.972,
    "subplots_bottom": 0.08,
}
os.environ["DA_HWC_AXIS_CONFIG_JSON"] = json.dumps({}, ensure_ascii=False)
os.environ["DA_HWC_PLOT_STYLE_JSON"] = json.dumps(LINEAR_PLOT_STYLE_CONFIG, ensure_ascii=False)
DA_NOTEBOOK_OUT_NAME_MAP = {
    ("linearbudget_chwcalendar31d", "beeswarm"): "Extended Data Fig8",
}
...
BASE_DIR = os.environ.get(
    "DA_BASE_DIR",
    str(CACHE_ROOT / "dynamic_adjustment" / "linear_budget"),
)
...
VARIANT_TAGS = _env_list("DA_VARIANT_TAGS", default=["linearbudget_chwcalendar31d"])
PLOT_GROUPS = _env_list("DA_LINEAR_PLOT_GROUPS", default=["three"])
LAYOUT_MODES = _env_list("DA_LAYOUT_MODES", default=["beeswarm"])
...
component_groups = {
    "two": {
        ...
        "suffix": "hwc_dyn_thermo",
    },
    "three": {
        "components": [
            ("dyn_linear", "dyn_linear_slope_per_year", "Dynamical"),
            ("thermo_linear", "thermo_linear_slope_per_year", "Thermodynamic"),
            ("deficit_linear", "deficit_linear_slope_per_year", "Deficit"),
        ],
        "suffix": "hwc_dyn_thermo_deficit",
    },
}
...
# 1.30：很大，可能会挤
# ============================================================
FONT_SCALE = float(os.environ.get("DA_FONT_SCALE", "1.18"))


def _scale_font(value):
    """按比例放大字号；兼容 int / float / 字符串数字。"""
    try:
        return float(value) * FONT_SCALE
    except Exception:
        return value
...
plt.rcParams.update(
    {
        ...
        "font.size": _scale_font(14),
        "axes.titlesize": _scale_font(18),
        "axes.labelsize": _scale_font(17),
        "xtick.labelsize": _scale_font(14),
        "ytick.labelsize": _scale_font(14),
        "axes.linewidth": 0.9,
    }
)
...
def _font_size(key, default):
    """读取 PLOT_STYLE 中的字号，如果没有则用 default，然后统一乘以 FONT_SCALE。"""
    return _scale_font(_style_value(key, default))
...
def _add_panel_label(ax, panel_idx):
    ax.text(
        ...
        ha="left",
        va="bottom",
        fontsize=_font_size("panel_label_fontsize", 18),
...
def _draw_reference_legend(ax, x_cols):
        ...
        ax.plot([0.02, 0.98], [0.945, 0.945], color="0.85", linewidth=0.8, transform=ax.transAxes, clip_on=False)
...
def _draw_member_key(ax, ordered_members, member_style_map):
        ...
        x_cols = np.array([0.06, 0.56])
        y_vals = np.linspace(0.82, 0.12, nrows_key)
...
def _panel_ylim(values):
    ...
    if finite.size == 0:
        ...
        ymin = vmin - span * 0.08
        ymax = vmax + span * 0.10
    rough_step = (ymax - ymin) / 4
    candidates = np.array([0.01, 0.02, 0.05, 0.1, 0.2, 0.25, 0.5, 1.0, 2.0])
...
def _plot_member_points(ax, region_rows, col_name, xi, layout_mode, member_style_map, member_offset_map):
    ...
    if layout_mode == "beeswarm":
        ...
        offsets = np.array(
            [member_offset_map.get(str(row_data["member"]), 0.0) for _, row_data in region_rows.iterrows()],
            dtype=float,
        )
    for offset, (_, row_data) in zip(offsets, region_rows.iterrows()):
        ...
        style = member_style_map.get(
            member_name,
            {"marker": "o", "color": generic_member_color, "linewidth": 1.35, "alpha": 0.9},
        )
...
def _draw_one_tag(tag, layout_mode, axis_override, group_name):
    ...
    figsize = _style_value("figure_size", [23.8, 18.2])
    ...
    outer = GridSpec(
        nrows=n_components,
        ncols=len(vars_order) + 1,
        width_ratios=[1.0, 1.0, 0.74],
        height_ratios=[1.0] * n_components,
        hspace=0.22,
        wspace=0.10,
        figure=fig,
    )
    main_axes = np.empty((n_components, len(vars_order)), dtype=object)
    band_axes = np.empty((n_components, len(vars_order)), dtype=object)
    for row_idx in range(n_components):
        for col_idx in range(len(vars_order)):
            inner = outer[row_idx, col_idx].subgridspec(2, 1, height_ratios=[5.1, 0.80], hspace=0.03)
    ...
    key_ax = fig.add_subplot(outer[:, 2])
    ...
    for row_idx, (comp_name, col_name, title) in enumerate(components):
        for col_idx, var_name in enumerate(vars_order):
            panel_idx = col_idx * n_components + row_idx
            ...
            ax.set_title(
                ...
                fontsize=_font_size("title_fontsize", 18),
                fontweight=_style_value("title_fontweight", "normal"),
            )
            _add_panel_label(ax, panel_idx)
            ax.tick_params(axis="both", labelsize=_font_size("main_tick_fontsize", 14))
            ax.tick_params(axis="x", labelbottom=False, labelsize=_font_size("main_xtick_fontsize", _style_value("main_tick_fontsize", 14)))
            ax.spines["top"].set_visible(False)
            ax.spines["right"].set_visible(False)
            if col_idx == 0:
                ax.set_ylabel(_style_value("ylabel", "Trend (?C yr$^{-1}$)"), labelpad=_style_value("ylabel_pad", 18), fontsize=_font_size("ylabel_fontsize", 17))
            ax.text(0.26, 0.955, _style_value("over_label", "Overestimated"), transform=ax.transAxes, ha="center", va="top", fontsize=_font_size("annotation_fontsize", 15.2), color="0.2")
            ax.text(0.77, 0.955, _style_value("under_label", "Underestimated"), transform=ax.transAxes, ha="center", va="top", fontsize=_font_size("annotation_fontsize", 15.2), color="0.2")

            band_ax.axvline(boundary_x, color="0.62", linestyle="--", linewidth=1.0)
            band_ax.set_ylim(0, 1)
            band_ax.set_yticks([])
            band_ax.set_xticks(x)
            band_ax.set_xticklabels(region_order, rotation=0, fontsize=_font_size("band_xtick_fontsize", 16))
            ...
            for xi, pct in enumerate(exceed_pcts):
                ...
                band_ax.text(xi, 0.49, pct_label, ha="center", va="center", fontsize=_font_size("band_value_fontsize", 13), color=text_color, fontweight="semibold")
            if col_idx == 0:
                band_ax.set_ylabel(_style_value("band_ylabel", "Models >\nERA5 (%)"), rotation=0, labelpad=_style_value("band_ylabel_pad", 25), fontsize=_font_size("band_ylabel_fontsize", 13.2), va="center")

    fig.subplots_adjust(left=_style_value("subplots_left", 0.055), right=_style_value("subplots_right", 0.965), top=_style_value("subplots_top", 0.972), bottom=_style_value("subplots_bottom", 0.08))
    out_name = f"DA_Ridge_daily_{tag}_region_trends_{group['suffix']}_modelstyle_{layout_mode}.png"
    ...
    for tag in VARIANT_TAGS:
        for group_name in PLOT_GROUPS:
            if group_name not in component_groups:
                raise ValueError(f"不支持的绘图组：{group_name}")
            for layout_mode in LAYOUT_MODES:
                saved.append(_draw_one_tag(tag, layout_mode, axis_override, group_name))

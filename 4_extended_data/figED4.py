...
ARGS, CONFIG = prepare_figure("figED4", "Reproduce Extended Data Figure 4.")
...
def main() -> None:
    ...
    require_files([source], figure="Extended Data Figure 4")
    dataset = xr.open_dataset(source)
    require_variables(dataset, EXTENDED_DATA_4_7_VARIABLES, source=str(source))
    fig = plt.figure(figsize=(16.0, 5.4), dpi=300)
    grid = fig.add_gridspec(3, 3, height_ratios=[1, 1, 0.03], hspace=0.06, wspace=0.07)
    contour = None
    for index, key in enumerate(EXTENDED_DATA_4_7_VARIABLES):
        contour = draw_ratio(
            fig.add_subplot(grid[index // 3, index % 3], projection=ccrs.PlateCarree()),
            dataset[key],
            "abcdef"[index],
            VARIABLE_TITLES[key],
            hide_y=index % 3 != 0,
    ...
    save_figure(fig, CONFIG.output_root, "Extended Data Fig4", dpi=300)
    plt.close(fig)
